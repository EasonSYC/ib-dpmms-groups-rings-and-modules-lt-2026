...
But note this formula may depend on the way in which we write the cosets \(g_1 H\) and \(g_2 H\), i.e. the choice of the particular elements elemenet \(g_1, g_2 \in G\).
...
\begin{definition}[Group Isomorphism]
    ...
    A group homomorphism \(\functiondomain{f}{G}{H}\) is an \emph{isomorphism} when it is a bijection. Two gropus are said to be \emph{isomorphic} if there exists an isomorphism between them, denoted as \(G \isomorphic H\).
\end{definition}
...
\begin{proof}
    We have already shown the normality part reviously.
...
\begin{proof}
    We first show \(HK\) is a subgroup. It certanily contains \(e = e \cdot e \in HK\) so it is non-empty. Let \(x = hk\) and \(y = h' k'\) for \(x, y \in HK\). We show that \(y x \inverse HK\). Observe that indeeed
    \begin{align*}
