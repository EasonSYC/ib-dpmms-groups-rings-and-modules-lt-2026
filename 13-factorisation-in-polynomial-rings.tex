...
\begin{definition}[Content of a Polynomial, Primitive Polynomials]
    ...
    \[
        \content\para{f} = \gcd\brce{a_0, \cdots, a_n} \in R.
    \]

    If \(\content\para{f}\) is a unit, then we say \(f\) is \emph{primitive}.
\end{definition}
...
\begin{proof}
    ...
    Now, suppose \(\content\para{fg}\) is not a unit, we may find some \(p \in R\), irreducible, with \(p \divides \content\para{fg}\). By the setup, we have \(p\notdivides \content\para{f}\), and \(p\notdivides \content\para{g}\).
...
\begin{corollary}[Contents are Multiplicative up to Units]
    ...
    Let \(R\) be a UFD and \(f, g \in R\brac{x}\). Then \(\content\para{fg}\) and \(\content\para{f} \cdot \content\para{g}\) are associates.
\end{corollary}

\begin{proof}
    Let
    \[
        f = \content\para{f} f_1, \quad g = \content\para{g} g_1
    \]
    for \(f_1, g_1 \in R\brac{x}\), primitive, then \(fg = \content\para{f} \content\para{g} f_1 g_1\), but \(f_1 g_1\) is primitive by \autoref{lem:gauss-lemma-primitive}.

    Since \(f_1, g_1\) are primitive, \(\content\para{f} \cdot \content\para{g}\) is indeed a gcd for the coefficients of \(fg\).
\end{proof}
...
\begin{proof}[of \autoref{thm:gauss-lemma} (Gauss's Lemma)]
    ...
    Now, we can write \(\para{ag} = \content\para{ag} g_1\) and \(\para{bh} = \content\para{bh} h_1\), with \(g, h\) primitive.

    We know since \(f\) is primitive, we have \(\content\para{abf} = ab\). By \autoref{cor:content-multiplicative}, we have
    \[
        ab = \content\para{abf} = \content\para{\para{ag} \cdot \para{bh}} = \content\para{ag} \cdot \content\para{bh} \cdot u
    \]
    where \(u\) is a unit. Also,
    \[
        abf = \content\para{ag} \cdot \content\para{bh} g_1 h_1 = u\inverse ab g_1 h_1,
    \]
...
\begin{proof}
    ...
    \[
        \para{bh} = \content\para{bh} \cdot h_1
    \]
    with \(h_1\) primitive. Since \(g\) is primitive, we see \(g h_1\) is primitive, so
    \[
        \content\para{bh} = u \cdot \content\para{bf}
    \] 
    where \(u\) is a unit. But \(\content\para{bf} = b \cdot \content\para{f}\), since \(b, f \in R\brac{x}\). Now, observe
    \[
        bf = g u b \cdot \content\para{f} \cdot h_1
    \]
...
\begin{proof}
    Let \(f \in R\brac{x}\). We let \(f = \content\para{f} \cdot f_1\).

    We may first factorise the content as
    \[
        \content\para{f} = p_1 \cdot p_2 \cdots p_n
    \]
...
\begin{examples}[More Examples of UFDs]
    ...
    Eamples of UFDs now include \(\ZZ\brac{x}, \RR\brac{x, y}, F\brac{x, y}\) for a field \(F\), and \(\ZZ\brac{x_1, \ldots, x_n}\). We may start from any UFD and construct a new UFD by taking the polynomial ring. (But recall that \(\ZZ\brac{\sqrt{-5}}\) is not a UFD.)
\end{examples}

\begin{theorem}[Eisenstein's Criteria]
    \label{thm:eisenstein}%
    Let \(R\) be a UFD, and \(f = a_0 + a_1 x + \cdots + a_n x^n \in R \brac{x}\) be primitive, and \(a_n \neq 0\). If there is some \(p \in R\) which is irreducible, such that
    \begin{enumerate}
        \item \(p \notdivides a_n\);
        \item \(p \divides a_i\) for all \(0 \leq i < n\); and
        \item \(p^2 \notdivides a_0\);
   \end{enumerate}
   then \(f\) is irreducible in \(R\brac{x}\), or equivalently, in \(F\brac{x}\), where \(F\) is a field of fractions of \(R\) (by \autoref{thm:gauss-lemma} (Gauss's Lemma)).
\end{theorem}
...
\begin{proof}
    ...
    Therefore, the only possible value of \(j\) is \(j = n\), and so \(k = n\). Hence, \(\deg\para{g} = n\) and \(\deg\para{h} = 0\), so \(f = gh\) is not a proper factorisation, because \(h\) is a unit. This finishes our proof.
\end{proof}

\begin{examples}[Application of Eisenstein's Criteria]
    \label{ex:application-of-eisenstein}%
    \begin{enumerate}
        \item We work with the integer ring \(R = \ZZ\). Take \(f = x^2 - p\) for any \(p \in \ZZ\) be a prime. By \autoref{thm:eisenstein} (Eisenstein's Criteria), this polynomial is irreducible (in both \(\ZZ\brac{x}\) and \(\QQ\brac{x}\)), and hence \(\sqrt[n]{p}\) is not rational.
        \item We work in \(\ZZ\brac{x}\) with the polynomial
        \[
            f\para{x} = x^{p - 1} + x^{p - 2} + \cdots + x + 1 = \frac{x^p - 1}{x - 1}
        \]
        where \(p\) is a prime number. This is called the \emph{\(p\)-cyclotomic polynomial}.

        The second form suggests us considering
        \[
            \hat{f} \para{y} = f\para{y + 1}.
        \]

        We can calculate \(\hat{f} \para{y}\) that
        \[
            \hat{f} \para{y} = y^{p - 1} + \binom{p}{1} y^{p - 2} + \cdots + \binom{p}{p - 1}.
        \]

        Now, this satisfies the criteria of \autoref{thm:eisenstein} (Eisenstein's Criteria), telling us that \(\hat{f}\) is irreducible, and hence \(f\) is irreducible. The final implication is as follows: say \(f\para{x} = f_1\para{x} f_2\para{x}\), then \(\hat{f} \para{y} = f\para{x - 1} = f_1\para{x - 1} f_2\para{x - 1}\) gives a factorisation.
