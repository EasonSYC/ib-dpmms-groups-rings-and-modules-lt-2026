...
\begin{corollary}[Prime Factorisation of Sum of Two Squares]
    \label{cor:int-sum-of-two-qaures}%
    An integer \(n\) can be written as a sum of squares \(x^2 + y^2\) if and only if \(n = p_1 ^{n_1} p_2^{n_2} \cdots p_k^{n_k}\) contains even powers of the \(4k + 3\) primes, i.e., if \(p_i \equiv 3 \pmod{4}\) then \(n_i\) is even.
\end{corollary}
