...
\begin{examples}[Examples of Integral Domains]
    ...
    so \(a = 0\).
        ...
        \item The rings \(\ZZ / 6\), \(\ZZ / \pqty{pq}\), \(\ZZ / 2 \times \ZZ / 2\), etc.
...
\begin{proof}
    \(R / I\) is a field if and only if its only ideals are the trivial ones, i.e., \(\Bqty{0}\) and \(R / I\) itself, by \autoref{prop:ideals-fields}.
...
\begin{proof}
    If \(I\) is prime, let \(\pqty{a + I}\) and \(\pqty{b + I}\) be cosets in \(R / I\) for some \(a, b \in R\). The product satisfies
    \[
...
\begin{proof}
    ...
    By \autoref{thm:first-isomorphism-theorem-ring} (First Isomorphism Theorem for Rings), we have \(\ZZ / n \ZZ \subring R\). Then \(\ZZ / n\ZZ\) is therefore integral, and hence \(\pqty{n}\) must be a prime ideal.
\end{proof}