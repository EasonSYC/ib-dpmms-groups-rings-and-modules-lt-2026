...
\begin{proposition}[Ideals of Fields]
    ...
    Let \(R\) be a ring. Then \(R\) is a field if and only if the only ideals of \(R\) are \(\brce{0}\) and \(R\)/
\end{proposition}

\begin{proof}
    Let \(R\) be a field and take some ideal \(I \idealsub R\). Say \(I \neq \brce{0}\). Then \(I\) contains a unit, say \(u \in I\). But then \(1 \in I\) since \(1 = uv\) for some \(v \in R\). So \(I = R\).

    Conversely, take \(0 \neq r \in R\). Then we have \(\para{r} = R\), and hence \(1 \in R\) gives \(1 = r \cdot s\) for some \(s \in R\). So \(r\) is a unit, and \(R\) is a field.
\end{proof}
...
\begin{proposition}[Ideals are Maximal if and only if Quotient is Field]
    ...
    An ideal \(I \idealsub R\) is maximal if and only if the quotient \(R / I\) is a field.
\end{proposition}

\begin{proof}
    \(R / I\) is a field if and only if its only ideals are the trivial ones, i.e., \(\brce{0}\) and \(R / I\) itself, by \autoref{prop:ideals-fields}.

    Using the correspondence result for ideals between \(R\) and \(R / I\) finishes the proof.
\end{proof}
...
\begin{proposition}[Ideals are Prime if and only if Quotient is Integral]
    ...
    An ideal \(I \idealsub R\) is prime if and only if \(R / I\) is an integral domain.
\end{proposition}

\begin{proof}
    If \(I\) is prime, let \(\para{a + I}\) and \(\para{b + I}\) be cosets in \(R / I\) for some \(a, b \in R\). The product satisfies
    \[
        \para{a + I} \cdot \para{b + I} = ab + I = 0 + I
    \]
    is the zero ideal if and only if \(ab \in I\). This, since \(I\) is prime, implies that \(a \in I\) or \(b \in I\), so \(a + I = 0 + I\) or \(b + I = 0 + I\), showing that \(R / I\) is indeed integral.

    Conversely, if \(R / I\) is integral, consider \(ab \in I\) where \(ab = 0\). Then
    \[
        ab + I = \para{a + I} \para{b + I} = 0 + I
    \]
    in \(R / I\) which is integral. Therefore, \(a + I = 0\) or \(b + I = 0\).
\end{proof}
...
\begin{remark}
    The converse is false. Consider the example \(\para{0} \idealsub \ZZ\), and \(\para{x} \idealsub \RR\brac{x, y}\) are both prime but not maximal.
\end{remark}
...
\begin{proof}
    Consider the unique homomorphism \(\functiondomain{f}{{\ZZ}}{R}\). Characteristic \(n\) implies that \(\para{n} \idealsub \ZZ\) is the kernel.

    By \autoref{thm:first-isomorphism-theorem-ring} (First Isomorphism Theorem for Rings), we have \(\ZZ / n \ZZ \subring R\). Then \(\ZZ / n\ZZ\) is therefore integral, and hence \(\para{n}\) must be a prime ideal.
\end{proof}